...
import { type ACUnit, type Organization, type Venue, type ACEvent } from '../../types';
...
export type DevicesManagementWorkspace = {
  orgs: Organization[];
  venues: Venue[];
  onUpdateDevice: (id: string, data: Partial<ACUnit>) => void;
  setShowAddDevice: (open: boolean) => void;
  expandedDeviceId: string | null;
  setExpandedDeviceId: (id: string | null) => void;
  deviceTempInputs: Record<string, string>;
  setDeviceTempInputs: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  setEditingDevice: (device: ACUnit | null) => void;
  setDeletingId: (id: string | null) => void;
  setDeleteType: (type: 'device' | 'user' | 'org' | 'venue' | null) => void;
  setDeleteError: (msg: string) => void;
  setShowAddEventModal: (open: boolean) => void;
  setEventDeviceId: (id: string) => void;
  setEventDeviceName: (name: string) => void;
  setEventOrganizationId: (id: string) => void;
  setEventVenueId: (id: string) => void;
  showAddDevice: boolean;
  selectedDeviceVenueId: string;
  setSelectedDeviceVenueId: (id: string) => void;
  subscribeDeviceEventAdd: (
    listener: (deviceId: string, event: ACEvent) => void
  ) => () => void;
  subscribeDeviceDeleted: (listener: (deviceId: string) => void) => () => void;
  subscribeDeviceUpdated: (listener: (device: ACUnit) => void) => () => void;
};

export type DevicesManagementPageProps = {
  workspace: DevicesManagementWorkspace;
  orgsLoading?: boolean;
  /** When true, skip manager org/venue API refresh (user already hydrated) */
  skipOrgFetch?: boolean;
...
/** Shared Device Management UI (manager + user) */
export function DevicesManagementPage({
  ...
  useEffect(() => {
    if (skipOrgFetch) return;
    void fetchMyOrganizations?.().catch(() => {});
    void fetchMyVenues?.().catch(() => {});
  }, [skipOrgFetch, fetchMyOrganizations, fetchMyVenues]);
  ...
  useEffect(() => {
    ...
    getVenuesByOrganization(selectedOrgId)
      .then((list) => {
        if (!active) return;
        const scoped = filterOrgVenues ? filterOrgVenues(list) : list;
        setOrgVenues(scoped);

        const deepLinkVenueId = pendingVenueDeepLink.current;
        const deepLinkMatch =
          deepLinkVenueId && scoped.some((v) => v.id === deepLinkVenueId)
            ? deepLinkVenueId
            : null;
        ...
        if (deepLinkMatch) {
          ...
          // Default: all venues in the selected organization (scoped)
          setSelectedVenueId(ALL_VENUES_ID);
        }
      })
      .catch(() => {
    ...
    return () => { active = false; };
  }, [selectedOrgId, setSelectedDeviceVenueId, filterOrgVenues]);
  ...
  useEffect(() => {
    ...
    fetchDevicesForSelection(selectedVenueId, orgVenues)
      .then((list) => {
        ...
        // Keep dashboard `units` in sync with real devices (drop mock units)
        if (setUnits) {
        setUnits((prev) => {
